    ...
    <>
      {cart.length === 0 ? (
        <div className="cart-empty">
          ...
          <Link to='/'><Button sx={{ color: 'green', marginTop: '25px' }}>Volver al Inicio</Button></Link>

        </div>
      ) : (
        <div className="cart-full">
          <div style={{ marginBottom: 15 }}>
            <h2 style={{ marginBottom: 5 }}>Total a pagar: $ {totalPurchase()} </h2>

            <Button sx={{ marginRight: '5px' }} variant="outlined" color="error"
              onClick={() => { clearCart() }}>Limpiar Carrito</Button>
            <Link to='/checkout'><Button variant="outlined" color="success">Finalizar Compra</Button></Link>
          </div>
          <hr />
              ...
              <div className='container-cart' key={products.id}>
                <div className='details-cart'>
                  <div className='big-img'>
                    <img style={{ width: '200px', height: '200px' }} src={`${products.img}`} alt='img product' />
                  </div>
